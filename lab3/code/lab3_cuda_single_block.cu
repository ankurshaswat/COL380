...
__global__ void JACOBI_SINGLE_BLOCK(int N, double *E, double *e, bool *changed,
                                    int *ind, double *S) {
  printf("Hi\n");
  ...
  __shared__ int state, m, k, l;
  __shared__ double c, s, t;

  int count = 0;

  for (i = tid; i < N * N; i += stride) {
    E[i] = ((i / N) == (i % N));
  }
  ...
  while (state > 0) {
    ...
    for (i = tid + k; i < l; i += stride) {
      ROTATE(k, i, i, l, c, s, S, N);
    }

    __syncthreads();

    for (i = tid + l; i < N; i += stride) {
      ROTATE(k, i, l, i, c, s, S, N);
    }
...
void JACOBI(int n, double *dev_E, double *dev_e, double *dev_S) {
  ...
  printf("hi");
  JACOBI_SINGLE_BLOCK<<<1, 1>>>(n, dev_E, dev_e, dev_changed, dev_ind, dev_S);
  // printf("%d %d\n", state, count);
